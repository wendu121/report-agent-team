<template>
  <div class="page settings-page">
    <PageHead
      icon="Shield"
      title="Gate 审核规则"
      sub="编辑三道闸的审核标准（gates/review.md）。保存后下一个任务即生效——引擎每次运行都现读这个文件，无需重启容器。"
    />
    <StatStrip :stats="stats" />

    <el-alert
      type="info" :closable="false" class="blk"
      title="重要：此单一文件同时驱动 GateA / GateB / GateC 三道闸"
      description="引擎 build_gate_system 对每道闸都读取同一份 review.md 全文件（再追加『你现在执行 GateX』），因此编辑本文件会影响全部三道闸。DESIGN 原设想的 per-gate 分文件路由，因引擎现实已调整为单文件编辑（详见 VERIFICATION §12 偏差说明）。"
    />
    ...
    <el-card shadow="never" class="blk">
      <template #header>
        <div class="blk-head">
          <span class="blk-title">选择资源</span>
          <span class="blk-sub">共 {{ gates.length }} 个可编辑规则文件（派生自 gates/ 目录，新增 md 即自动出现）</span>
        </div>
      </template>
      <div v-loading="loading">
        <el-radio-group v-model="current" size="large" @change="onSwitch">
          <el-radio-button v-for="g in gates" :key="g.name" :value="g.name">
            {{ g.title || g.name }}
          </el-radio-button>
        </el-radio-group>
        <div v-if="meta" class="ag-meta">
          文件 <code>gates/{{ current }}.md</code> ·
          {{ meta.chars }} 字符 / {{ meta.lines }} 行 ·
          最后更新 {{ fmtTime(meta.updated) }}
        </div>
        <div v-if="meta?.headings?.length" class="ag-toc">
          <span class="ag-hint">本文含分节：</span>
          <el-tag
            v-for="h in meta.headings" :key="h" size="small" type="info" effect="plain"
            class="ag-toc-item"
          >{{ h.replace(/^##\s*/, '') }}</el-tag>
        </div>
      </div>
    ...
    <div v-loading="loading" class="ag-split">
      <el-card shadow="never" class="ag-pane blk">
        ...
        <textarea
          ...
          class="ag-editor"
          spellcheck="false"
          placeholder="在此编辑 Gate 审核规则（Markdown）"
...
const meta = computed(() => gates.value.find((g) => g.name === current.value));
const dirty = computed(() => content.value !== original.value);

const stats = computed(() => [
  { label: '可编辑规则', value: gates.value.length, tone: 'brand' as const, hint: 'gates/*.md' },
  { label: '当前字符数', value: meta.value?.chars ?? 0, tone: 'muted' as const, hint: '单文件驱动三道闸' },
  {
    label: '校验',
...
const errors = computed<string[]>(() => {
  ...
  if (!c.trim()) errs.push('审核规则内容不能为空（清空会导致三道闸失去行为定义）');
...
async function load(): Promise<void> {
  ...
  try {
    ...
    if (!current.value && gates.value.length) current.value = gates.value[0].name;
    await loadContent();
  } catch (e) {
...
async function loadContent(): Promise<void> {
  ...
  try {
    const { data } = await api.get(`/admin/gates/${encodeURIComponent(current.value)}`);
    content.value = data.content ?? '';
    original.value = content.value;
    await loadAudit();
  } catch (e) {
    ElMessage.error(`读取规则失败：${(e as Error).message}`);
  }
}
...
async function loadAudit(): Promise<void> {
  ...
  try {
    const { data } = await api.get(
      `/admin/gates/${encodeURIComponent(current.value)}/audit`,
      { params: { limit: 10 } }
    );
...
async function onSwitch(): Promise<void> {
  if (dirty.value) {
    try {
      await ElMessageBox.confirm('当前规则有未保存的修改，切换后将丢失。确定切换？', '未保存', {
        type: 'warning',
...
async function save(): Promise<void> {
  ...
  saving.value = true;
  try {
    const { data } = await api.put(`/admin/gates/${encodeURIComponent(current.value)}`, {
...
.ag-toc { margin-top: 10px; display: flex; flex-wrap: wrap; align-items: center; gap: 6px; }
.ag-toc-item { cursor: default; }
